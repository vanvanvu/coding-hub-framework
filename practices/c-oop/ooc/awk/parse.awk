...
function setMethod (class, m,    d, l) {
	...
	if (Name[d] ~ /AtConstruct$/)	# Constructor method
		Replace["`ctor"] = 1
	else
		Replace["`ctor"] = 0
...
function methodDeclaration (class, line, linkage,    n, i, v, m, d, self) {
	...
	{
		for (i = 2; i <= n; ++ i) 
			if (Name[d = declarator(v[i])] == "class")
			{	if (self)
					error(m " ?duplicate class")
				if (! As[d])
					error(m " ?class without @")
				if (Type[d] != "")
					error(m " ?class with foreign class")
				self = i
			}
